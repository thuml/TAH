...
void QuantizationLossLayer<Dtype>::Forward_gpu(
  ...
  //std::ofstream fout("qlossout.txt", ios::out);
...
__global__ void BackwardGPU(const int nthreads, const int num, const int dim, 
        const Dtype* bottom_data, const Dtype scale,const Dtype threshold, Dtype* bottom_diff) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    if ((threshold > 0) && ((abs(bottom_data[index])- 1) > threshold))
    {
    ...
    {
	if(bottom_data[index] >= 0){
	    //bottom_diff[index] = tanh(abs(bottom_data[index]) - 1);
            bottom_diff[index] = scale * tanh(abs(bottom_data[index]) - 1);
    	}
    	else{
	    //bottom_diff[index] = -1 * tanh(abs(bottom_data[index]) - 1);
            bottom_diff[index] = -scale * tanh(abs(bottom_data[index]) - 1);
    	}
    }
  }
  ...
  if (propagate_down[0]) {
    ...
    BackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, outer_num_, inner_num_, bottom_data, 
                loss_weight_ / (outer_num_ * inner_num_), threshold_, bottom_diff);
    //std::ofstream fout("qdiffout.txt", ios::out);
    //caffe_gpu_memcpy(sizeof(Dtype) * bottom[0]->count(), bottom[0]->gpu_diff(), bottom[0]->mutable_cpu_diff());
